...
method takes two arguments: forecasts the next `h` (for horizon) and
`level`.
...
The
[`forecast`](https://Nixtla.github.io/statsforecast/src/ets.html#forecast)
is inteded to be compatible with distributed clusters, so it does not
store any model parameters. If you want to store parameter for
everymodel you can use the `fit` and `predict` methods. However, those
methods are not defined for distrubed engines like Spark, Ray or Dask.
